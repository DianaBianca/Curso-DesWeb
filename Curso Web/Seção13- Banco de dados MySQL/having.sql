...
#HAVING - Filtro realizado sobre os resultados de agrupamentos (group by)// o having nao existe sem o group by

select  estado , count(*) as alunos_por_estado
from tb_alunos
group by estado;
...
from tb_alunos
group by estado
having alunos_por_estado >= 3;

select  estado , count(*) as alunos_por_estado
from tb_alunos
group by estado
having estado in('CE','SC') and alunos_por_estado > 2;
